...
inline void partitionOpenMPClusters(Point centroids[], int c_count,
                                    Point points[], int p_count, Point *c_temp,
                                    int *cluster_amt, bool resetClusters) {
  // We assume that c_sum and c_amt are initialized to zeros!
  int thread_count = omp_get_max_threads();
  std::vector<std::vector<Point>> local_c_temp(
      thread_count,
      std::vector<Point>(c_count, initialize_point(0, 0, 0, 0, 0, 0, 0)));
  std::vector<std::vector<int>> local_cluster_amt(thread_count,
                                                  std::vector<int>(c_count, 0));
    ...
    for (int p_i = 0; p_i < p_count; p_i++) {
      ...
      if (dist < current_point->minDist) {
        ...
        if (current_point->cluster != -1) {
          local_c_temp[thread_id][current_point->cluster].sub_point(
              current_point);
...
int main() {
  ...
  // 3. Perform Kmeans Clustering and time how long it takes, and print the time
  // to the console.
